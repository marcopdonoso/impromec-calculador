...
export default function SaveAndCalc({ 
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const sectorId = (hasSectors && activeSector && activeSector.id) ? activeSector.id : null
      ...
      console.log('SaveAndCalc - Usando valores actualizados:', { 
        ...
        originalTrayType: hasSectors ? activeSector?.trayTypeSelection : currentProject?.trayTypeSelection,
        originalReservePercentage: hasSectors ? activeSector?.reservePercentage : currentProject?.reservePercentage
      });

      // Llamar al servicio para calcular la bandeja
      const result = await calculateTray(projectId, sectorId, trayType, reservePercentage)
